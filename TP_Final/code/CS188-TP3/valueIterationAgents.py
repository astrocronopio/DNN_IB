...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeActionFromValues(self, state):
        ...
        ###############################################
        """   
        policy = util.Counter() #Como en el  .py anterior
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        estadoss = self.mdp.getStates()


        for i in range(self.iterations): # every k

            state = estadoss[i % len(estadoss)]

            if self.mdp.isTerminal(state):
                continue

            acciones = self.mdp.getPossibleActions(state)
            optimal = max([self.getQValue(state,action) for action in acciones])
