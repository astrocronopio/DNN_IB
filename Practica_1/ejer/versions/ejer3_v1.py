...
class knearest(object):
    ...
    def predecir(self, x, k=1):
        ...
        #norm = np.linalg.norm(self.x -x.reshape(x.shape[0],self.x.shape[1]), axis=2)
        #yp[:]= self.y[np.argmin(norm, axis=1)]
        for idx in range(x.shape[0]):
            norm = np.linalg.norm(self.x - x[idx].ravel(), axis=1)
            #idmin= np.argmin(norm)
            idmin = np.argpartition(norm,k)[:k] #lo k menores valores
            #yp[idx] = self.y[idmin]
            kvecinos = self.y[idmin].ravel() 
            print(kvecinos)
            clasificacion = np.bincount(kvecinos)#Cuenta cuantas veces se repite cada vecino
            yp[idx] = np.argmax(clasificacion)
            

        return  yp

def knn_implementacion(data, prueba):
    ...
    yp=model.predecir(x_test[:prueba])

    #print(yp)
    #print(y_test[:prueba].ravel())
    counter=0
...
def ejer3():
    ...
    knn_implementacion(datasets.cifar10.load_data(), 20)#ejer3_mnist()
    pass
